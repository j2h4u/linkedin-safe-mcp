...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="linkedin-safe-mcp-packaging-") as tmp:
        workdir = Path(tmp)
        ...
        executable = venv_dir / "bin" / "linkedin-safe-mcp"
        ...
        _run([str(executable), "status"])

    print("packaging smoke passed: wheel built, installed, CLI help and status ran")
    return 0
