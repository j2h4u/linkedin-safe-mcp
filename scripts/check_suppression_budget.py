...
CONFIG_TABLE = ("tool", "linkedin_mcp", "suppressions")
...
def _config(root: Path) -> dict[str, object]:
    ...
    for key in CONFIG_TABLE:
        if not isinstance(current, dict) or key not in current:
            return {}
        current = current[key]
    return _expect_table(current, "[tool.linkedin_mcp.suppressions] must be a table")


def _scan_paths(root: Path, config: dict[str, object]) -> list[Path]:
    raw_paths = config.get("paths", DEFAULT_SCAN_PATHS)
    if not isinstance(raw_paths, list):
        raise TypeError("[tool.linkedin_mcp.suppressions].paths must be a list")
...
def _baseline(config: dict[str, object]) -> dict[str, int]:
    ...
    if not isinstance(raw_baseline, dict):
        raise TypeError("[tool.linkedin_mcp.suppressions.baseline] must be a table")
