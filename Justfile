...
print-lint:
    uv run ruff check --preview --select T20 src/linkedin_mcp
...
typecheck:
    uv run basedpyright src/linkedin_mcp scripts
...
coverage:
    uv run pytest --cov=src/linkedin_mcp --cov-report=term-missing

# Human CRAP report over the full suite.
crap:
    uv run pytest --cov=src/linkedin_mcp --cov-report=term-missing --crap --crap-threshold=30 --crap-top-n=30

# Hard CRAP gate: every function must stay at or below CRAP 30.
crap-check:
    coverage_file="$(mktemp /tmp/linkedin-safe-mcp-crap-coverage.XXXXXX.json)"; \
    trap 'rm -f "$coverage_file"' EXIT; \
    uv run pytest --cov=src/linkedin_mcp --cov-report=json:"$coverage_file"; \
    uv run python -m scripts.crap_gate --coverage "$coverage_file" --src src/linkedin_mcp --threshold 30
...
docker-build: docker-check
    docker build -t linkedin-safe-mcp:local .

# Recreate the local Docker service.
docker-up:
    docker compose up -d --force-recreate --remove-orphans --wait --wait-timeout 90

# CI-safe runtime smoke: build, start, wait for health, exercise Streamable HTTP, and clean up.
runtime-smoke:
    #!/usr/bin/env bash
    set -euo pipefail
